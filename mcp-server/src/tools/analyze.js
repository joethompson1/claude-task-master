		...
		server.addTool({
			...
			parameters: z.object({
				...
				parentKey: z
					...
					.optional()
					.describe(
						'Parent Jira issue key to filter tasks'
					),
				research: z
					.boolean()
					.optional()
			...
			execute: async (args, { log, session }) => {
				try {
					...
					if (result.success) {
						log.info(`Jira task complexity analysis complete: ${result.data.message}`);
					} else {
						log.error(
					...
					return handleApiResult(result, log, 'Error analyzing Jira task complexity');
				} catch (error) {
