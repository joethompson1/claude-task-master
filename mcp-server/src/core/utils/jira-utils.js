...
export async function fetchJiraTaskDetails(taskId, withSubtasks = false, log) {
	try {		
		...
		const issueResult = await jiraClient.fetchIssue(taskId, { log, expand: true });
		...
		if (withSubtasks) {
			try {
				...
				log.warn(`Could not fetch subtasks for ${taskId}: ${subtaskError.message}`);
...
export async function fetchTasksFromJira(parentKey, withSubtasks = false, log) {
	try {
		...
		if (parentKey) {
			...
			log.info(`Fetching Jira subtasks for parent ${parentKey} with JQL: ${jql}`);
		} else {
		...
		const tasks = await Promise.all(searchResult.data.map(async (jiraTicket) => {
			...
			if (withSubtasks && jiraTicket.jiraKey) {
				...
				try {
					...
					const subtasksResult = await fetchTasksFromJira(jiraTicket.jiraKey, false, log);
					if (subtasksResult && subtasksResult.tasks) {
						task.subtasks = subtasksResult.tasks;
						log.info(`Added ${task.subtasks.length} subtasks to ${jiraTicket.jiraKey}`);
					}
				} catch (subtaskError) {
					log.warn(`Error fetching subtasks for ${jiraTicket.jiraKey}: ${subtaskError.message}`);
					// Continue without subtasks - this is not fatal
				}
			}
		...
		if (withSubtasks) {
			...
			subtaskStats.completed = allSubtasks.filter(t => t.status === 'done' || t.status === 'completed').length;
			subtaskStats.inProgress = allSubtasks.filter(t => t.status === 'in-progress').length;
			subtaskStats.pending = allSubtasks.filter(t => t.status === 'pending').length;
			subtaskStats.blocked = allSubtasks.filter(t => t.status === 'blocked').length;
			subtaskStats.deferred = allSubtasks.filter(t => t.status === 'deferred').length;
			subtaskStats.cancelled = allSubtasks.filter(t => t.status === 'cancelled').length;
			subtaskStats.completionPercentage = subtaskStats.total > 0 ? 
				(subtaskStats.completed / subtaskStats.total) * 100 : 0;
		}
...
export async function createJiraIssue(jiraTicket, log) {
	try {
		...
		try {
			...
			const response = await client.post('/rest/api/3/issue', jiraTicket.toJiraRequestData());
			...
			const isPriorityError = 
				requestError.response?.data?.errors?.priority === "Field 'priority' cannot be set. It is not on the appropriate screen, or unknown.";
			
			// If it's a priority field error and we included priority, retry without it
			if (isPriorityError && jiraTicket.priority) {
				log.warn(`Priority field error detected: ${requestError.response.data.errors.priority}`);
				...
				try {
					...
					const client = jiraClient.getClient();
					const retryResponse = await client.post('/rest/api/3/issue', requestBody);
		...
		// Log the error
		const issueTypeDisplay = jiraTicket.issueType === 'Subtask' ? 'subtask' : jiraTicket.issueType.toLowerCase();
		...
		const errorMessage = [
			...
			error.response?.data?.errorMessages ? error.response.data.errorMessages.join(', ') : error.message,
	...
	try {
		...
		for (const id of taskIds) {
			...
			try {
				...
				const transitionResult = await jiraClient.transitionIssue(id, newStatus, { log });
				
				if (!transitionResult.success) {
					throw new Error(transitionResult.error?.message || 'Transition failed');
				}
				
				log.info(`Successfully updated Jira issue ${id} status to "${newStatus}"`);
...
export async function findNextJiraTask(parentKey, log) {
	try {
		...
		if (result && result.tasks) {
			...
			log.info(`Found ${allTasks.length} tasks ${parentKey ? `for parent ${parentKey}` : 'in total'}`);
		}
		...
		if (eligibleTasks.length === 0) {
			log.info('No eligible tasks found - all tasks are either completed or have unsatisfied dependencies');
			return {
				success: true,
				data: {
					nextTask: null,
		...
		log.info(`Found next task: ${nextTask.id} - ${nextTask.title} (${nextTask.priority} priority)`);
...
export async function updateJiraIssues(issueIds, prompt, useResearch = false, options = {}) {
    ...
    try {
        ...
        if (!issueIdArray.length) {
            throw new Error('Missing required parameter: issueIds must be a non-empty string or array');
        }
        ...
        for (let i = 0; i < updates.length; i++) {
            ...
            try {
                ...
                if (isSubtask) {
                    ...
                    const fullIssueResponse = await jiraClient.fetchIssue(update.id, { log });
                    if (!fullIssueResponse.success) {
                        log.warn(`Failed to fetch full issue details: ${fullIssueResponse.error?.message}`);
                        continue;
                    }
                ...
                if (Object.keys(requestData.fields).length > 0) {
                    try {
                        ...
                        if (Object.keys(requestData.fields).length > 0) {
                            ...
                            log.info(`Updated issue ${update.id} fields: ${Object.keys(requestData.fields).join(', ')}`);
                        } else {
                        ...
                        if (updateError.response && updateError.response.data) {
                            log.error(`API error details: ${JSON.stringify(updateError.response.data)}`);
                            
                            // If there are specific field errors, log them and try again without those fields
                            if (updateError.response.data.errors) {
                                Object.entries(updateError.response.data.errors).forEach(([field, error]) => {
                                ...
                                if (Object.keys(requestData.fields).length > 0) {
                                    ...
                                    log.info(`Updated issue ${update.id} with remaining fields: ${Object.keys(requestData.fields).join(', ')}`);
                                }
                            }
                        }
                    }
                ...
                const originalTask = tasks.find(task => 
                    task.id === update.id || task.jiraKey === update.id
                );
                ...
                if (originalTask.title !== update.title) changesApplied.push({
                    field: 'summary',
                    old: originalTask.title,
                    new: update.title
                });
                if (originalTask.description !== update.description) changesApplied.push({
                    field: 'description',
                    old: originalTask.description,
                    new: update.description
                });
                if (originalTask.priority !== update.priority) changesApplied.push({
                    field: 'priority',
                    old: originalTask.priority,
                    new: update.priority
                });
                if (originalTask.implementationDetails !== update.implementationDetails) changesApplied.push({
                    field: 'implementationDetails',
                    old: originalTask.implementationDetails,
                    new: update.implementationDetails
                });
                if (originalTask.acceptanceCriteria !== update.acceptanceCriteria) changesApplied.push({
                    field: 'acceptanceCriteria',
                    old: originalTask.acceptanceCriteria,
                    new: update.acceptanceCriteria
                });
                if (originalTask.testStrategyTdd !== update.testStrategyTdd) changesApplied.push({
                    field: 'testStrategy',
...
export async function expandJiraTask(taskId, numSubtasks, useResearch = false, additionalContext = '', options = {}) {
	...
	try {
		...
		if (!taskDetails.success) {
			throw new Error(`Failed to fetch Jira task: ${taskDetails.error?.message || 'Unknown error'}`);
		}
		...
		report(`Successfully generated ${generatedSubtasks.length} subtasks. Creating in Jira...`);
		...
		for (let i = 0; i < generatedSubtasks.length; i++) {
			...
			try {
				...
				report(`Creating subtask ${i+1}/${generatedSubtasks.length}: ${subtask.title}`);
	...
	try {
		...
		if (convert) {
			...
			try {
				...
				let epicKey = null;
				if (parentKey) {
					log.info(`Fetching parent task ${parentKey} to check for epic relationship...`);
					...
					if (parentResult.success) {
						...
						if (parent.parentKey) {
							log.info(`Parent task has ${parent.dependencies.length} dependencies/links`);
							epicKey = parent.parentKey;
							log.info(`Found potential epic relationship: ${epicKey}`);
						}
					} else {
						log.warn(`Could not fetch parent task: ${parentResult.error?.message || 'Unknown error'}`);
					}
				}
...
export async function removeJiraTask(taskId, log) {
    try {
        ...
        try {
            ...
            if (subtasksResult.success && subtasksResult.tasks) {
                ...
                if (subtasks.length > 0) {
                    log.info(`Task ${taskId} has ${subtasks.length} subtasks that need to be removed first`);
                }
            }
        } catch (error) {
        ...
        if (subtasks.length > 0) {
            log.info(`Deleting ${subtasks.length} subtasks before removing parent task ${taskId}`);
            ...
            for (const subtask of subtasks) {
                try {
                    ...
                    if (subtaskResult.success) {
                        ...
                        log.warn(`Failed to remove subtask ${subtask.id}: ${subtaskResult.error?.message || 'Unknown error'}`);
                    }
                } catch (subtaskError) {
                    subtaskResults.push({
                        id: subtask.id,
                    ...
                    log.error(`Error removing subtask ${subtask.id}: ${subtaskError.message}`);
                }
            }
            ...
            try {
                const remainingSubtasksResult = await fetchTasksFromJira(taskId, false, log);
                if (remainingSubtasksResult.success && 
                    ...
                    log.warn(`There are still ${remainingCount} subtasks remaining that could not be deleted`);
                }
            } catch (error) {
        ...
        try {
            ...
            if (error.response && 
                ...
                error.response.data && 
                (error.response.data.errorMessages || []).some(msg => msg.includes('subtask'))) {
                return {
                    success: false,
                    error: {
 ...
 */
export async function analyzeJiraTaskComplexity(parentKey, threshold = 5, useResearch = false, outputPath, options = {}, log, context = {}) {
	try {
		// Check if Jira is enabled using the JiraClient
		const jiraClient = new JiraClient();
		...
		log.info(`Analyzing complexity of Jira tasks ${parentKey ? `for parent ${parentKey}` : 'in project'}`);
		...
		if (filteredTasks.length === 0) {
			return {
				...
				error: {
					...
					message: 'No active tasks found to analyze (all tasks are completed, cancelled, or deferred)'
				}
			};
		}
		
		log.info(`Analyzing ${filteredTasks.length} active tasks (skipping ${tasksResult.tasks.length - filteredTasks.length} completed/cancelled/deferred tasks)`);
		...
		const { analyzeTaskComplexity } = await import('../../../../scripts/modules/task-manager.js');
